<script>
  ...
  onMount(() => {
    if (localStorage.getItem('admin_token')) goto('/flags');
  });
  ...
  async function handleSignup() {
    ...
    loading = true;
    try {
      const res = await fetch(`${PUBLIC_API_URL}/api/auth/signup`, {
        method: 'POST',
      ...
      localStorage.setItem('admin_org_id', data.organizationId);
      localStorage.setItem('admin_org_name', data.organizationName);
      goto('/flags');
    } catch (e) {
      error = e.message;
    } finally {
  ...
  <div class="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-700 via-violet-800 to-indigo-900 flex-col justify-between p-12">
    ...
    <div>
      ...
      <p class="text-violet-200 text-base leading-relaxed mb-10">
        Register as an Organization Admin to start managing feature flags for your team.
      </p>
      <div class="bg-white/10 backdrop-blur rounded-2xl p-5 border border-white/20">
        <p class="text-xs font-semibold text-violet-200 uppercase tracking-wide mb-3">How it works</p>
        <div class="space-y-3">
          {#each [
            ['1', 'Get your Organization ID from your Super Admin'],
            ['2', 'Create your admin account with that ID'],
            ['3', 'Start managing feature flags instantly']
          ] as [num, text]}
  ...
  <div class="w-full lg:w-1/2 flex items-center justify-center p-8 bg-gray-50">
    <div class="w-full max-w-sm">
      ...
      <div class="mb-8">
        ...
        <p class="text-gray-500 text-sm mt-1">Register to manage your organization's flags</p>
      </div>
      ...
      <form on:submit|preventDefault={handleSignup} class="space-y-5">
        ...
        <div>
          ...
          <p class="text-xs text-gray-400 mt-1.5 flex items-center gap-1">
            ...
            Provided by your Super Admin from the organizations list
          </p>
        </div>
